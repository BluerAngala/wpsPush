/*
    ...
    推送：bark、pushplus、Server酱（ServerChan）、邮箱（email）
          钉钉（dingtalk）、discord、企业微信（qywx）、息知（xizhi）
          即时达（jishida）、wxpusher
*/
...
function getMessage(){
  ...
  if (flagConfig == 1) {
    ...
    for (let i = 2; i <= 100; i++) {
      ...
      msgDict.update = Application.Range("C" + i).Text;     // 脚本更新时间，即脚本是否已执行
      msgDict.msg = Application.Range("D" + i).Text;     // 待推送消息
      msgDict.date = Application.Range("E" + i).Text;     // 推送时间，即单天是否已推送
      msgDict.methodPush = Application.Range("F" + i).Text;     // 推送方式
      msgDict.flagPush = Application.Range("G" + i).Text;     // 是否通知
      msgDict.pool = Application.Range("H" + i).Text;     // 是否加入消息池，加入消息池的都会整合为一条消息统一推送
...
function sendNotify(){
  ...
  for (let i = 0; i < msgArray.length; i++) {
    ...
    if(msgCurrentDict.pool == "否" && msgCurrentDict.flagPush == "是" && msgCurrentDict.update != msgCurrentDict.date && msgCurrentDict.msg != "" && msgCurrentDict.date != todayDate){ // 时间不一致说明未推送。消息为空不进行推送。今天未推送
      ...
      // 写入推送的时间
      Application.Range("E" + (i + 2)).Value = todayDate
...
function pushUnit(message, key, name){
  ...
  try{
    ...
    } else if (name == "ServerChan" || name == "Server酱") {
      serverchan(message, key);
    } else if (name == "email") {
      email(message);
    } else if (name == "dingtalk" || name == "钉钉") {
      dingtalk(message, key);
    } else if (name == "discord") {
      discord(message, key);
    }else if (name == "qywx" || name == "企业微信"){
      qywx(message, key);
    } else if (name == "xizhi" || name == "息知") {
      xizhi(message, key);
    }else if (name == "jishida" || name == "即时达"){
      jishida(message, key);
    }else if (name == "wxpusher" || name == "WxPusher"){
